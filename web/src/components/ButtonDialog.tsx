...
export function ButtonDialog({ buttonTitle, dialogTitle, icon, children }: ButtonProps) {
  return (
    <Dialog.Root>
      <Dialog.Trigger
        type="button"
      ...
      >
        {/* {React.createElement(icon, { size: 20, className: 'text-violet-500' })} */}
        {icon}
        {buttonTitle}
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content className='absolute p-10 bg-zinc-900 rounded-2xl w-full max-w-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
          ...
          <Dialog.Title className='text-3xl leading-tight font-extrabold'>{dialogTitle}</Dialog.Title>
          {children}
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
